  ...
  *{{box-sizing:border-box}}
  ...
  .mono{{font-family:ui-monospace,Menlo,monospace}}
    ...
    if sprint:
        ...
        # comprometido = suma determinista de los puntos de las historias del sprint
        # (sprint-plan.json no lleva el campo; se calcula desde backlog.json).
        committed = sum(smap.get(sid, {}).get("estimate") or 0
                        for sid in sel
                        if isinstance(smap.get(sid, {}).get("estimate"), int))
        A(f'<div class="sub mono" style="margin-top:6px">capacidad {sprint.get("capacity","?")} pts · comprometido {committed} pts</div>')
